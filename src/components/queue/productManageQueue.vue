		...
		<el-dialog :title="actionState" :visible.sync="dialogFormVisible">
		  <el-form :model="editForm" label-width="112px" :rules="editFormRules" ref="editForm" class="editForm">
		    <el-form-item label="商品名称：" prop="name">
				...
				<el-form-item label="国际条形码：" prop="bar_code">
					<el-input v-model="editForm.bar_code" auto-complete="off"></el-input>
				</el-form-item>
				...
				<el-form-item label="进货价：" prop="cost_price">
					<el-input v-model="editForm.cost_price" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="销售价：" prop="sell_price">
					<el-input v-model="editForm.sell_price" auto-complete="off"></el-input>
				</el-form-item>
...
<script type="text/ecmascript-6">
	...
	import { getGoodslist, submitGood, deleteWare, addWare, getKey, goodOnOff, waresChsort } from 'api/api'
  ...
  export default {
  	data() {
  		return {
  			...
  			arr: ['name', 'good_code', 'bar_code', 'spec', 'unit', 'cost_price', 'sell_price'],
  			good_id: null,
  			displayorder: null,
  			editForm: {
          name: '',
          ...
          if (valid) {
          	this.dialogFormVisible = false
						...
						if (this.actionStatus === 1) {
							...
							params.pics = JSON.stringify(this.editForm.goodsPics)
						...
						if (this.actionStatus === 2) {
							...
							params.pics = JSON.stringify(_this.editForm.goodsPics)
							params.introduce = _this.imgHTML
							addWare(params).then((res) => {
								console.log(res)
								if (res.code !== 200) {
									_this.$notify.error({
										title: '提示',
			...
			dialogOpen(good) {
				...
				if (good.pics === "") {
					...
					this.editForm.goodsPics = JSON.parse(good.pics)
				}
				// 商品详情图片
            ...
            .then((res) => {
              ...
              _this.editForm.goodsPics.push(newPic)
              _this.editForm.logo = _this.editForm.goodsPics[0].url
              _this.good.logo = _this.editForm.goodsPics[0].url
              console.log('商品图片列表：')
